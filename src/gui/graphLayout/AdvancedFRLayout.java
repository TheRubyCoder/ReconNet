 ...
 * license; see either "license.txt" or
...
public class AdvancedFRLayout<V, E>
  ...
  private Map<V, Integer> vertexAges = LazyMap.decorate(
    new HashMap<V, Integer>(), new Factory<Integer>() {
   ...
   */
  public AdvancedFRLayout(Graph<V, E> g) {
    super(g);
  }
  ...
  protected synchronized void calcPositions(V v) {
    ...
    double ageFactor = ((minAgeFactor - 1) / (maxAge - 1)) * vertexAges.get(v)
      + 1 - ((minAgeFactor - 1) / (maxAge - 1));
    ...
    double newXDisp = fvd.getX() / deltaLength * Math.min(deltaLength,
      (temperature * ageFactor));
    ...
    double newYDisp = fvd.getY() / deltaLength * Math.min(deltaLength,
      (temperature * ageFactor));
    ...
    try {
      for (V v2 : getGraph().getVertices()) {
        ...
        if (v1 != v2) {
          ...
          fvd1.offset((xDelta / deltaLength) * force, (yDelta / deltaLength)
            * force);
        }
      }
    } catch (ConcurrentModificationException cme) {
  ...
  private void ageAll() {
    ...
    for (V v : getGraph().getVertices()) {
      int age = vertexAges.get(v);
      if (age < maxAge) {
        vertexAges.put(v, age + 1);
      }
    }
  }
  ...
  public boolean done() {
    ...
    if (frozen || currentIteration > mMaxIterations || temperature < 1.0
      / max_dimension) {
      if (!frozen) {
        ageAll();
      }
