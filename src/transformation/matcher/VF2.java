...
public final class VF2 {
	...
	/**
	 * This PRNG is used to realize the nondeterminism of the matching process. To avoid repeating
	 * match sequences (by repeated call of getMatch) this PRNG is used over the entire lifetime 
	 * of this matcher instance.
	 */
	private final Random RANDOM;
	
	/**
	 * This PRNG is used over all instances, where no specific PRNG is given. If more than one matcher 
	 * instance is created in quick succession, the same PRN sequence can be generated by all of them. 
	 * This PRNG exists to avoid this problem. This PRNG isn't intended for local use (except of getInstance).
	 */
	...
	/**
	 * Index of source candidate node in the array that is returned from the get candidate methods.
	 */
	private final byte INDEX_SOURCE_CANDIDATE = 0;

	/**
	 * Index of target candidate node in the array that is returned from the get candidate methods.
	 */
	private final byte INDEX_TARGET_CANDIDATE = 1;

	/**
	 * NULL candidate pair as start value for the candidate search process and for indicating, that 
	 * there are no more valid pairs.  
	 */
	...
	/**
	 * gets the next place candidate pair, which fulfills the condition implied by the given candidate set.
	 * The candidate has the same sourceIndex, as the given one, and a higher targetIndex. So it is possible 
	 * to iterate over all supporting targetIndexes for a given sourceIndex.
	 * 
	 * @param  previousCancidatePair  previous candidate pair, from which a new candidate pair should be searched 
	 *   							  or CORE_NULL_NODE_CANDIDATE to indicate, that a new search is to be started   							  
	 * @param  nextCandidateSet       the candidate set, to be searched
	 * 
	 * @return CORE_NULL_NODE_CANDIDATE to indicate, that no pair was found or a valid pair otherwise
	 */
	private int[] getNextPlaceCandidatePair(int[] previousCancidatePair, NEXT_CANDIDATE_SET nextCandidateSet) {
		assert nextCandidateSet == NEXT_CANDIDATE_SET.TERMINAL_OUT
			|| nextCandidateSet == NEXT_CANDIDATE_SET.TERMINAL_IN
			|| nextCandidateSet == NEXT_CANDIDATE_SET.NEW;
		...
		if (previousSourceIndex != CORE_NULL_NODE && previousTargetIndex >= coreTargetPlaces.length - 1) {
			// the given candidate is already the last possible candidate for this sourceIndex
		...
		if (sourceIndex == CORE_NULL_NODE) {
			// CORE_NULL_NODE_CANDIDATE given, look for the first valid sourceIndex
		...
		// search for the next valid targetIndex, starting with the initialized targetIndex
		if (nextCandidateSet == NEXT_CANDIDATE_SET.TERMINAL_OUT) {
	...
	/**
	 * gets the next transition candidate pair, which fulfills the condition implied by the given candidate set.
	 * The candidate has the same sourceIndex, as the given one, and a higher targetIndex. So it is possible 
	 * to iterate over all supporting targetIndexes for a given sourceIndex.
	 * 
	 * @param  previousCancidatePair  previous candidate pair, from which a new candidate pair should be searched 
	 ...
	 * @return CORE_NULL_NODE_CANDIDATE to indicate, that no pair was found or a valid pair otherwise
	 */
	private int[] getNextTransitionCandidatePair(int[] previousCancidatePair, NEXT_CANDIDATE_SET nextCandidateSet) {
		assert nextCandidateSet == NEXT_CANDIDATE_SET.TERMINAL_OUT
			|| nextCandidateSet == NEXT_CANDIDATE_SET.TERMINAL_IN
			|| nextCandidateSet == NEXT_CANDIDATE_SET.NEW;
	 ...
	 * @param  startIndex   index, at which the search begins
	 * 
