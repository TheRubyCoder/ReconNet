...
public abstract class SessionDataAbstract implements SessionData {
	...
	protected void checkContaining(Petrinet petrinet, JungData jungData) {
		...
		check(graph.getEdgeCount() == petrinet.getAllArcs().size(), "to many arcs");
		check(graph.getVertexCount() == (petrinet.getAllTransitions().size() + petrinet.getAllPlaces().size() ), "to many nodes");
